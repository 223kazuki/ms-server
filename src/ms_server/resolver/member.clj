(ns ms-server.resolver.member
  (:require [integrant.core :as ig]))

(def members [{:member/name "加藤 延夫"
...
(defmethod ig/init-key :ms-server.resolver.member/get-members [_ _]
  (fn [context arguments value]
    (let [{:keys [grade]} arguments]
      (cond->> members
        grade (filter #(= (:member/grade %) grade))
        true  (map (fn [member] (->> member
                                     (map #(hash-map (keyword (name (key %))) (val %)))
                                     (into {}))))))))
